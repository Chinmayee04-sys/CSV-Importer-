...
const COLUMN_ALIASES = {
  name: ['name', 'full name', 'lead name', 'contact name', 'first name', 'last name', 'customer', 'customer name', 'client', 'client name', 'person name', 'candidate', 'prospect', 'lead', 'applicant', 'username', 'user name', 'person', 'individual'],
  email: ['email', 'e-mail', 'email address', 'email id', 'mail', 'e mail', 'email_id', 'emailaddress', 'email_addr', 'recipient', 'recipient email'],
  mobile: ['mobile', 'phone', 'phone number', 'mobile number', 'contact number', 'tel', 'telephone', 'phone_no', 'phone no', 'contact_no', 'cell', 'cell phone', 'whatsapp', 'phone1', 'phone 1', 'mobile1', 'mobile 1', 'primary phone', 'primary mobile', 'contact'],
...
function scanAllColumns(record) {
  ...
  for (const val of Object.values(record)) {
    ...
    if (digits.length >= 10 && !isDate) allPhones.push({ raw: val, digits });
  }
...
function extractRecordsFallback(records) {
  return records.map(record => {
    const name = findColumn(record, COLUMN_ALIASES.name) || '';
    ...
    const dateRaw = findColumn(record, COLUMN_ALIASES.date) || '';
